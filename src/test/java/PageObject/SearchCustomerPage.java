...
public class SearchCustomerPage {
	...
	//(//table[@class='table table-bordered table-hover table-striped dataTable no-footer'])[1]
	...
	public boolean searchCustomerByEmail(String email)
	{
		...
		for(int i=1;i<=ttlRows;i++)//to iterate all the rows of the grid
		{
			...
			Predicate<String> p1= j->j.equalsIgnoreCase(actualEmailAdd);
			p1.test(email);
			found=true;
...
//			}
	...
	public boolean searchCustomerByName(String name)
	{
		...
		for(int i=1;i<=ttlRows;i++)//to iterate all the rows of the grid
		{
			...
			Predicate<String> p1 = j->j.equalsIgnoreCase(actualName);
			p1.test(name);
			found=true;
			break;
